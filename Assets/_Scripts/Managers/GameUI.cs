...
public class GameUI : MonoBehaviour
{
    ...
    void Start()
    {
        startBB = gameManager.blackShirkens;
    }
    ...
    public void WinScreen()
    {
        ...
        if (gameManager.blackShirkens >= startBB)
        {
            goodJobText.text = "FANTASTIC!";
            StartCoroutine(Stars(3));
        }
        else if (gameManager.blackShirkens >= startBB - (gameManager.blackShirkens / 2))
        {
            goodJobText.text = "AWESOME!";
            StartCoroutine(Stars(2));
        }
        else if (gameManager.blackShirkens > 0)
        {
